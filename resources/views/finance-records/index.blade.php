        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <div class="d-inline-block align-middle">
                    <h1 class="display-6 fw-bold text-dark mb-0">Finance</h1>
                </div>
            </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-2">
                            <select name="payment_type" class="form-select">
                                ...
                                <option value="tuition" {{ request('payment_type') == 'tuition' ? 'selected' : '' }}>
                                    Tuition</option>
                                <option value="fee" {{ request('payment_type') == 'fee' ? 'selected' : '' }}>Fee
                                </option>
                                <option value="fine" {{ request('payment_type') == 'fine' ? 'selected' : '' }}>Fine
                                </option>
                            </select>
                        </div>
                        <div class="col-md-2">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @forelse($financeRecords as $index => $record)
                                <tr>
                                    <td>{{ ($financeRecords->currentPage() - 1) * $financeRecords->perPage() + $index + 1 }}
                                    </td>
                                    <td>
                                        <a href="{{ route('finance-records.show', $record) }}"
                                    ...
                                    <td>
                                        <span
                                            class="badge bg-{{ $record->payment_type == 'tuition' ? 'primary' : ($record->payment_type == 'fee' ? 'info' : 'warning') }} rounded-pill px-3 py-2">
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a class="dropdown-item"
                                                        href="{{ route('finance-records.show', $record) }}"><i
                                                ...
                                                @if ($record->payment_status == 'pending')
                                                    <li>
                                                        <a class="dropdown-item text-success"
                                                            href="{{ route('finance-records.markAsPaid', $record) }}">
                                                ...
                                                <li>
                                                    <form method="POST"
                                                        action="{{ route('finance-records.destroy', $record) }}"
                                                        class="d-inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="dropdown-item text-danger"
                                                            onclick="return confirm('Are you sure you want to delete this finance record?')">
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="text-center py-5">
                                        ...
                                        <a href="{{ route('finance-records.create') }}" class="btn btn-primary">Add New
                                            Record</a>
                                    </td>
                                </tr>
                            @endforelse
