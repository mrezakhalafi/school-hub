                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <div class="d-inline-block align-middle">
                            <h1 class="display-6 fw-bold text-dark mb-0">Office Boys</h1>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @forelse($officeBoys as $index => $boy)
                                        <tr>
                                            <td>{{ ($officeBoys->currentPage() - 1) * $officeBoys->perPage() + $index + 1 }}
                                            </td>
                                            <td>
                                                @if ($boy->profile_image)
                                                    <img src="{{ asset('storage/' . $boy->profile_image) }}" alt="Profile"
                                                        width="50" height="50" class="rounded-circle">
                                                @else
                                                    <img src="https://picsum.photos/100/100?random={{ $boy->id }}"
                                            ...
                                            <td>
                                                <a href="{{ route('office-boys.show', $boy) }}" class="btn btn-sm btn-info"
                                                    title="View">
                                                    <i class="fas fa-eye"></i> View
                                                </a>
                                                <a href="{{ route('office-boys.edit', $boy) }}"
                                                    class="btn btn-sm btn-warning" title="Edit">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="{{ route('office-boys.qr', $boy) }}"
                                                    class="btn btn-sm btn-primary" title="QR Code">
                                                    <i class="fas fa-qrcode"></i> QR
                                                </a>
                                                <button type="button" class="btn btn-sm btn-danger"
                                                    onclick="deleteBoy({{ $boy->id }})" title="Delete">
