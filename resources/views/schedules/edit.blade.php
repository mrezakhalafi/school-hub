...
<div class="container">
    <div class="row">
        <div class="col-12">
            <h1 class="h3 mb-4">Edit Schedule for {{ $class->name }}</h1>
        </div>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('schedules.update', [$class->id, $schedule->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="day" id="day" class="form-select @error('day') is-invalid @enderror" required>
                                        ...
                                        @foreach($days as $dayOption)
                                            <option value="{{ $dayOption }}" {{ old('day', $schedule->day) == $dayOption ? 'selected' : '' }}>
                                                {{ $dayOption }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="hour" id="hour" class="form-select @error('hour') is-invalid @enderror" required>
                                        ...
                                        @foreach($hours as $hourOption)
                                            <option value="{{ $hourOption }}" {{ old('hour', $schedule->hour) == $hourOption ? 'selected' : '' }}>
                                                {{ $hourOption }}:00
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="subject" id="subject" class="form-control @error('subject') is-invalid @enderror" 
                                   value="{{ old('subject', $schedule->subject) }}" required>
                            @error('subject')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
